...
def encode_text(vid_caps, model, text_model, tokenizer, idf_dict, device):
    ...
    return text_features, mask, idf_weights, word_features
...
def em_cos_score(
    ...
    for batch_start in iter_range:
        ...
        for i, sen in enumerate(sen_batch):
            ...
            # For special tokens, use [SOS] and [EOS]
            '''
            local_emb = word_embs[i, 0:sequence_len]#].unsqueeze(0)#, 0:sequence_len]
            global_emb = word_embs[i, sequence_len-1]#].unsqueeze(0)#, sequence_len-1]
