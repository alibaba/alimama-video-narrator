    ...
    def compute_cider(self):
        ...
        for test, refs in zip(self.ctest, self.crefs):
            ...
            for ref in refs:
                ...
                t_score = sim(vec, vec_ref, norm, norm_ref, length, length_ref)
                score=max(np.mean(t_score),score)
                #for t_i,t_s in enumerate(t_score):
                #    score[t_i]=max(score[t_i],t_score[t_i])
                #score += sim(vec, vec_ref, norm, norm_ref, length, length_ref)
            # change by vrama91 - mean of ngram scores, instead of sum
            #print(score)
            score_avg = score#np.mean(score)
